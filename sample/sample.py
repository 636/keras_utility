...
from keras_utility.trainer import KerasTrainer
from keras_utility.store import DirectoryModelStore
...
def train(trainer: KerasTrainer,
    ...
    store = DirectoryModelStore(Path('./models'), 'mnist')
    model_key = store.new_model_key()
    store.save(model_key, model)
